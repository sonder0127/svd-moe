...
# 设置环境变量以使用阿里云的 Hugging Face 镜像源
# os.environ['HF_DATASETS_CACHE'] = '/path/to/local/cache'
# os.environ['TRANSFORMERS_CACHE'] = '/path/to/local/cache'
# os.environ['HF_HOME'] = '/path/to/local/cache'
# os.environ['HUGGINGFACE_HUB_CACHE'] = '/path/to/local/cache'
# os.environ['HF_DATASETS_HUB_ENDPOINT'] = 'https://huggingface.co/datasets'
# os.environ['HUGGINGFACE_HUB_URL_TEMPLATE'] = 'https://huggingface.co/datasets/{repo_id}/resolve/{revision}/{filename}'
...
def get_calib_train_data(name, tokenizer, nsamples, seqlen=2048, seed=3, batch_size=1, dataset_cache_dir=None):
    import random
    random.seed(seed)
    #dataset_cache_dir={"./outfile/wikitext/wikitext-2-raw-v1/"}
    cache_file = (
    ...
    if name == "c4":
        ...
        # traindata = load_dataset("wikitext", "wikitext-2-raw-v1", cache_dir=dataset_cache_dir, split="train")
        # tot_text = "\n\n".join(traindata["text"])
...
def get_test_data_local(name, tokenizer, seq_len=2048, batch_size = 4):
    class IndexDataset(Dataset):
        def __init__(self, tensors):
            self.tensors = tensors
